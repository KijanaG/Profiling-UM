...
void execute(UM UM, SegMem SegMem)
{
    while (true)
    {
        ...
        run_instruct(UM, SegMem, 
        *(uint32_t *)UArray_at((UArray_T)Seq_get(SegMem->seg_mem, SEG_ZERO), 
        UM->prog_counter),
                     &UM->prog_counter);
        UM->prog_counter++;
    }
                  ...
                  uint32_t *prog_counter)
{
    ...
    uint32_t *regC = NULL;
    if (opcode <= 6) {
        A = (uint32_t)Bitpack_getu(instruction, ID_LEN, A_LSB);
        ...
        if (opcode <= 10) {
            if (opcode <= 8) {
                if (opcode == 7) {
                    ...
                    regB = &((UM->registers)[B]);
                    regC = &((UM->registers)[C]);
                    UArray_T mapped_seg = create_segment(*regC);
            ...
            if (opcode <= 12) {
                ...
                // load_value(UM, instruction);
                    uint32_t A = (uint32_t)Bitpack_getu(instruction, ID_LEN, 25);
